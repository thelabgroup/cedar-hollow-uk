...
(function () {
  ...
  function cardHtml(item) {
    ...
    return (
      ...
      '<div class="cabin-item_body">' +
      ratingHtml(item, '<div class="margin-bottom margin-xsmall"><p class="text-size-small">', "</p></div>") +
      '<a href="' +
  ...
  // Ratings are only rendered when a listing actually carries one. We have no
  // verified per-property rating or review data, so nothing is invented here.
  function ratingHtml(item, open, close, withStars) {
    if (typeof item.rating !== "number" || typeof item.reviews !== "number") return "";
    return (
      open +
      (withStars ? starsFor(item.rating) + " " : "★ ") +
